...
|---|---|---|---|
| `paper` | Page background, default node fill | `#ffffff` (white — 흰 문서 삽입용) | `#2d3142` (jet-black) |
...
- **Paper is pure white (`#ffffff`) for this project**: 산출물이 흰 배경 문서에 삽입되므로 paper·마스크·배경을 순백으로 맞춘다. 회색/크림 paper는 흰 문서 위에서 다이어그램만 블록으로 떠 보임. (다른 프로젝트의 기본값은 warm-neutral이지만 여기서는 순백 우선.)
