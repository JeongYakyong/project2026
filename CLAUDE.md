...
## ★ 최상위 규칙 — 보고서용 다이어그램·이미지 작도 (위반 금지)
보고서·문서용 다이어그램/이미지를 만들기 전 **반드시 `9. design/drawing_rule.md`를 먼저 읽고 그대로 따른다(SSOT).** 핵심: **흰 배경(#ffffff)·문서용 가로 좁게(max-width 720px)·데이터/라벨을 상단에 분리해 그것만 수정하는 HTML 구성·맑은 고딕 시스템 폰트(외부 폰트 금지).** 저장 위치는 `9. design/report/stageN/`(단계별 폴더 생성). 만든 뒤 `_render.py`로 PNG 렌더→직접 Read 검수. 기준 산출물: `9. design/report/stage1/*.html`·`pipeline_overview.html`. **회색 배경·가로로 넓은 캔버스·손코딩 좌표·excalidraw/외부 스킬 양식으로 임의 제작 금지.**
