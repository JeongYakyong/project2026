...
- forecast  : KIMG-land 5 지점을 _common KIMG core 로 메모리 long DF 로 받아
              (fetch_kimg_land_long) SOLAR_RAD/TCLD/MIDLOW_CLOUD/TEMP_C/WIND_U_V_10M/
              REH/RAIN_CONV/RAIN_STRAT 를 radiation/total_cloud/midlow_cloud/temp/
              wind_spd_10m·wd_sin_10m·wd_cos_10m/reh/rainfall 로 피벗 (제주와 달리
              KIMR-land 가 없어 KIMG temp/wind 도 유일 예보 weather; reh/rainfall 은
              2026-06-08 추가 -- RAIN_* 누적값은 per-base diff 로 시간당 강수 변환).
...
# REH(습도) 는 별도 처리(round 2, reh_<지점>), RAIN_* 는 누적->시간차 변환이 필요해
# _KIMG_RAIN_CATS 로 따로 뺀다 (둘 다 _KIMG_SCALAR 단순 pivot 대상이 아님).
_KIMG_SCALAR = {
    "SOLAR_RAD":    "radiation",
    "TCLD":         "total_cloud",
...
# 누적 강수 카테고리 (rainc_acc/rainl_acc 유래).  per-base diff 로 시간당 강수 변환.
_KIMG_RAIN_CATS = ("RAIN_CONV", "RAIN_STRAT")
...
def kimg_land_long_to_wide(df: pd.DataFrame) -> pd.DataFrame:
    ...
    지점별로 처리한다 (제주 kimr_one_point 과 동일 구조):
      - 비강수(SOLAR_RAD/TCLD/MIDLOW_CLOUD/TEMP_C/WIND_U_V_10M/REH): (fcst_datetime,
        category) 별 freshest base 를 골라 pivot.  WIND_U/V_10M 은 spd/sin/cos 분해,
        REH 는 reh_<지점>(%, round 2).
      - 강수(RAIN_CONV/RAIN_STRAT): 누적값이라 freshest 전에 per-base 로 (conv+strat)
        합산 -> 시간차 diff(>=0) 로 시간당 강수 변환한 뒤 freshest -> rainfall_<지점>.
        (KIMR rain 처리 및 api_fetchers_nouse.fetch_kma_future_ncm 의 acc->diff 와 동일.)
    """
    if df.empty:
        return pd.DataFrame()
    df = df.copy()
    ...
    for point, suffix in POINT_SUFFIX.items():
        ...
        # 1) 비강수: (fcst_datetime, category) 별 freshest -> pivot.
        non_rain = sub[~sub["category"].isin(_KIMG_RAIN_CATS)]
        non_rain = freshest(non_rain, ["fcst_datetime", "category"])
        piv = non_rain.pivot(index="fcst_datetime", columns="category", values="fcst_value")
