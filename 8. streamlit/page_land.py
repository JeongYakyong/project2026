...
def render_longhorizon():
    """장지평 — 표준 네비(시작일) + 같은 행 끝 날짜 슬라이더(최대 14일 창) + 하단 캡션."""
    ...
    # 예측 길이 — 네비 행 오른쪽 슬라이더, 끝 날짜 그대로 표시 (14일 창, 미래엔 D+표기 보조)
    ...
    st.caption(f"{start:%Y-%m-%d} ~ {end:%Y-%m-%d} ({n_days}일) · 시작일 과거 선택 가능 — "
               "과거 구간은 발행 시점(D+k) 비교 모드로 전환됩니다")
    ...
    if past_mode:
        ...
        st.caption(f"발행 기준: {ORIGIN:%Y-%m-%d} 23:00 (사전 적재). 미래 구간은 실측이 없어 예측만 표시됩니다. "
                   "예보 범위 밖 기상은 (월,시) 기후값 폴백이 섞일 수 있습니다. "
...
def render_forecast_menu():
    ...
    cap.caption(f"{start:%Y-%m-%d} ~ {end:%Y-%m-%d} ({n}일) · "
                "실측 ━ / 예측 ··· / KPX DA ╌")
...
def render_data_status():
    ...
    # segmented_control은 재클릭으로 해제(None)될 수 있어 기본값 폴백을 둔다.
    c1, c2, c3 = st.columns([1.3, 2.4, 2.3], vertical_alignment="bottom")
    table = c1.segmented_control("테이블", ["historical", "forecast"],
                                 default="historical", key="ds_table") or "historical"
    psel = c2.segmented_control("조회 기간(과거)", list(PAST_PRESET) + ["직접 선택"],
                                default="1개월", key="ds_past") or "1개월"
    if psel == "직접 선택":
        ...
        if table == "forecast":
            fsel = c3.segmented_control("미래 지평", list(FUT_PRESET),
                                        default="적재 끝까지", key="ds_fut") or "적재 끝까지"
