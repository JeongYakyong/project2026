...
**최종 산출물 — 발전용 가스(핵심) + 도시가스(보조·참고)** (2026-06-21 확장, G-26): 위 명제의 결과물은 **발전용 가스 송출량 예측**이다. 여기에 **도시가스 송출량(난방 중심·기온 기반)**을 **보조·참고**로 더해 '전국 가스 송출량'의 그림을 더 완성한다(10단계). 도시가스는 net_load·신재생과 무관한 별개 동인(기온)이라 **§1.2 명제의 일부가 아니라**, 데모의 일일 송출량을 풍부하게 하는 보조 계열이다. Streamlit 일일 송출량 화면에 **도시가스 on/off 토글**을 두어, 켜면 일간 집계에 도시가스를 더해 '총 송출량'으로 본다(기본 화면=발전용 시간당, 상세 §4 보조 트랙·§6.4).
