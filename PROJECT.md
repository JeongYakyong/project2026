...
> - **모델**: 수요=**PatchTST v4(anchor-residual + recent-climatology), 지평별 하이브리드**(G-24: D1-6=head 파인튜닝 무보정·D7-15=기존+post-hoc 보정. 옛 lgbm-하이브리드는 G-23 폐기) / 신재생 solar=**PatchTST(D1-6)+신규 LGBM(D7-15) 블렌딩**·wind=**신규 LGBM**(G-27 장지평 재훈련: 지평인지+평년앵커) / 가스=**LGBM 자기회귀 v3(최근 원전 관성 피처 + walk-forward, G-25)** + 기후값 블렌딩(calib 는 OFF — v3 가 bias 구조 제거).
...
- 남음: 서버 배포(가중치 scp+백필) 사용자 수동. **★다음 세션 과제: 7단계 가스 재교정 여부 점검**(신재생 변동 가스 전파는 +7MW로 미미하나 정직 재검증).
