...
- **8-A ✅(전국)**: 확정 디자인 v2(`CONCEPT_8-0.md` §3.4) — 멀티 페이지(전국/제주) + 메뉴(종합/수요 예측/데이터 현황/SMP[제주만]). `app.py`·`common.py`·`page_land.py`·`page_jeju.py`. 전국 전 메뉴 동작(KPX sukub 실시간 포함), 제주는 골격+데이터 현황만.
- 작업 쪼개기: 8-B 제주(종합·수요·SMP, 선행=제주 서빙 백필) → 8-C 검증·KOGAS 탭 → 8-D brief_ai → 8-E 배포·시연 영상.
