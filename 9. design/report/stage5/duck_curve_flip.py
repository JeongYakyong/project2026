...
for y in YEARS2:
    if y in norm.columns:
        # 매핑된 색상 가져오기 (매핑 안 된 연도는 검정색 처리 등 예외 방어)
        c = color_map.get(y, '#000000') 
        
        ax[0].plot(norm.index, norm[y], color=c, lw=2.4 if y >= 2025 else 1.4,
...
ax[0].annotate('한낮\n전기\n수요', (12.5, ax[0].get_ylim()[1]-15), ha='center', fontsize=10, color="#000000")
ax[0].annotate('새벽\n전기\n수요', (3.5, ax[0].get_ylim()[1]-15), ha='center', fontsize=10, color="#000000")
...
for off, vals, lab in [(-w / 2, apr, '4월'), (w / 2, may, '5월')]:
    ...
    for b, v in zip(bs, vals):
        # 양수면 위로 3포인트, 음수면 아래로 3포인트 띄움 (원하는 여백만큼 숫자 조절)
        y_offset = 0.5 if v >= 0 else -2
...
fig.suptitle('낮과 밤이 뒤바뀐 전력수요 - 한낮 수요가 새벽보다 낮다', fontsize=20)
